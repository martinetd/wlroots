...
static void seat_handle_bind(struct wl_client *client, void *_wlr_seat,
	...
	if (new_seat_client) {
		...
		wl_list_init(&seat_client->binds);
	...
	wl_list_insert(&seat_client->binds, wl_resource_get_link(wl_resource));
...
void wlr_seat_destroy(struct wlr_seat *seat) {
	...
	wl_list_for_each_safe(client, tmp, &seat->clients, link) {
		...
		wl_resource_for_each_safe(resource, next_resource, &client->binds) {
...
struct wl_client *wl_client_for_wlr_seat_client(
	...
	// All resources point to the same client by definition
	struct wl_resource *resource = NULL;
	wl_resource_for_each(resource, &wlr_seat_client->binds) {
		break;
	}
	if (!resource) {
		return NULL;
...
void wlr_seat_set_capabilities(struct wlr_seat *wlr_seat,
	...
	wl_list_for_each(client, &wlr_seat->clients, link) {
		...
		struct wl_resource *resource;
		wl_resource_for_each(resource, &client->binds) {
